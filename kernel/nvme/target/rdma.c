...
static u16 nvmet_rdma_map_sgl_inline(struct nvmet_rdma_rsp *rsp)
{
	...
	if (off + len > NVMET_CMD_CAPSULE_SIZE - sizeof(struct nvme_command)) {
	...
	sg_init_one(&rsp->cmd->inline_sg, rsp->cmd->inline_data + off, len);
		...
		struct nvme_keyed_sgl_desc *sgl, bool invalidate)
{
	...
	ret = rdma_rw_ctx_init(&rsp->rw, rsp->queue->cm_id->qp,
			rsp->queue->cm_id->port_num, sg, sg_cnt, 0, addr, key,
...
static int nvmet_rdma_add_port(struct nvmet_addr *addr)
{
	struct rdma_cm_id *cm_id;
	...
	ret = kstrtou16(addr->disc_addr.trsvcid, 0, &port);
	if (ret)
		return ret;

	addr_in.sin_family = AF_INET;
	addr_in.sin_addr.s_addr = in_aton(addr->disc_addr.traddr);
	addr_in.sin_port = htons(port);

	cm_id = rdma_create_id(&init_net, nvmet_rdma_cm_handler, NULL,
	...
	pr_info("enabling port %d (%pISpc)\n",
		le16_to_cpu(addr->disc_addr.portid), &addr_in);
	addr->priv = cm_id;
	return 0;
...
static void nvmet_rdma_remove_port(struct nvmet_addr *addr)
{
	struct rdma_cm_id *cm_id = addr->priv;
