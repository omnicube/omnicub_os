...
static void nvmet_execute_admin_connect(struct nvmet_req *req)
{
	...
	if (d->cntlid != cpu_to_le16(0xffff)) {
		pr_warn("connect attempt for invalid controller ID\n");
		status = NVME_SC_CONNECT_CTRL_BUSY | NVME_SC_DNR;
		goto out;
	}

	status = nvmet_alloc_ctrl(d->subsysnqn, d->hostnqn, &ctrl);
	if (status)
		goto out;
